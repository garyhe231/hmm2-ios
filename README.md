...
**症状**：手动存档时如果文件名含中文（不改名时默认名就是中文），点「确定」后游戏直接
退出到 DOS 提示符，进度丢失。

**根因**：iOS 文件系统要求文件名是合法 UTF-8，而游戏用 GBK 编码。存档时它尝试创建
`.\GAMES\<中文>.GM1`，那串 GBK 字节不是合法 UTF-8，创建失败。这个中文版处理"存档
失败"的方式就是打一行错误然后终止 —— 所以**不是崩溃**，DOSBox 全程不打任何异常，
查起来很容易走偏。

**怎么确认的**：
- 退出瞬间 DOS 屏幕上有一行 GBK 乱码，路径部分认得出是 `.\GAMES\...GM1`
- 设备上 `Documents/HEROES2/GAMES/` 里 `AUTOSAVE.GM1` 是游戏自己写成功的 ——
  目录存在、可写、磁盘没满，唯一区别就是它是纯英文名
- 用纯英文名 `TEST1` 手动存档，正常

**绕过**：存档时用英文/数字名。

**根治（未做）**：在 DOSBox 的本地磁盘层加一层文件名编码转换，DOS 侧 GBK ↔ 主机侧
UTF-8。创建、打开、改名、删除、目录遍历都得成对处理，否则存了读不出来。

## 已知问题：存档框里回车无效

屏幕键盘的 ↵ 在存档命名框里没反应，得点「确定」。键盘通路本身是好的（同一个框里能
打字），映射也对（`key-enter` → `SDL_SCANCODE_RETURN`，见 `DPKeyBinding.m`），
所以更可能是这个中文版的对话框本身不接受回车确认。影响很小，未深查。
